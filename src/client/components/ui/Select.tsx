    ...
    return (
      ...
      >
        <BaseSelect.Trigger
          ref={ref}
          className={cn(
            "inline-flex w-full items-center justify-between border border-mist-2 bg-paper text-left text-ink shadow-sm outline-none transition-colors",
            ...
            "data-[popup-open]:border-flame-400",
            SIZES[size],
            className
          )}
        >
          <BaseSelect.Value className="block truncate" placeholder={placeholder} />
          <BaseSelect.Icon className="ml-2 shrink-0 text-ink-3">
        ...
        <BaseSelect.Portal>
          <BaseSelect.Positioner sideOffset={4} className="z-50 outline-none">
            <BaseSelect.Popup
              className={cn(
                "max-h-[var(--available-height)] min-w-[var(--anchor-width)] overflow-y-auto rounded-md border border-mist-2 bg-paper p-1 text-ink shadow-xl outline-none"
              )}
            >
              <BaseSelect.List>
                {options.map((option) => (
                  <BaseSelect.Item
                    ...
                    className={cn(
                      ...
                      "data-[highlighted]:bg-flame-100 data-[highlighted]:text-ink data-[disabled]:pointer-events-none data-[disabled]:opacity-50"
                    )}
                  >
