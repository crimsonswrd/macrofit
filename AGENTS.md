...
## THIS APP: FORMETRA — calorie / macro tracker for athletes

Russian-language UI. Design identity is **established** (see `DESIGN.md`):
white/red "sport-hall bold" — Oswald display + Manrope body, `flame-*` red accent,
`ink*`/`mist*` neutrals. Shared UI tokens in `components/ui/_shared/variants.ts`
were retuned to this palette (solid+neutral = ink, solid+primary = flame).
Fonts are loaded via `<link>` in `src/client/index.html` — **do not** use
`@import url(...)` in `index.css`, Tailwind hoists it after its own output and
PostCSS errors.

### Backend — `src/server/nutrition/`
- `db.ts` — two Stores:
  - `foods` — global curated food DB. Macros **per 100 g/ml**. Fields: `name`
    (unique), `searchName` (lowercased, for regex search), `category`,
    `calories/protein/carbs/fat`, `unit` (`г`|`мл`), `isVerified`, `createdAt`.
  - `foodEntries` — one logged item: `userId`, `date` (`YYYY-MM-DD`, user's local
    calendar day), `meal` (`breakfast|lunch|dinner|snack`), `foodId`, `foodName`,
    `unit`, `grams`, and **snapshotted** macro values so food-DB edits don't
    rewrite history.
- `foodSeed.ts` — `FOOD_CATEGORIES` + `seedFoods` (125 curated products across 13
  categories, values from standard nutrient tables). Add products here.
- `index.ts` — module `nutrition`:
  - queries: `getCategories`, `searchFoods({query, category, limit})`,
    `getRecentFoods`, `getDay({date})` (returns `{items, totals, targets}`).
  - mutations: `addEntry`, `updateEntryGrams`, `deleteEntry`.
  - `DEFAULT_TARGETS` = 2600 kcal / 165 P / 300 C / 80 F — **hardcoded for now**;
    per-user goal calculation (TDEE) is not built yet.
- Migration v2 `seedFoodDatabase` (`src/server/migrations/seedFoods.ts`) upserts
  `seedFoods` by `name` — idempotent, re-runs safely; edit seed values and bump
  nothing, it syncs on every deploy of that migration version only, so add a new
  migration version if you need a re-sync.

### Frontend
- `src/client/lib/nutrition.ts` — shared types (`Food`, `DiaryItem`, `DayData`),
  meal labels, local-date helpers (`todayKey`, `shiftDateKey`, `formatDateKey`,
  `formatWeekday`), `fmt()` ru number formatting, `scaleMacros()`.
- `pages/HomePage.tsx` — `Diary` when logged in (date switcher, `DaySummary`,
  4 meal cards, delete entry), marketing `Landing` when not.
- `pages/FoodsPage.tsx` (`/foods`) — searchable/filterable read-only food table.
- `components/nutrition/DaySummary.tsx` — calorie + macro progress bars.
- `components/nutrition/AddFoodDialog.tsx` — two-step dialog: search/filter list →
  portion (grams) step with live macro preview.
- `components/Page.tsx` — app shell: sticky header with wordmark + desktop nav,
  bottom tab bar on mobile. NAV list lives here; add new routes there.
- Starter example pages, store, query and cron were deleted. Local/sandbox
  convenience login now uses the narrowly scoped server-only `demoAuth` module;
  it returns no credentials in production and exposes no public config.

### Not built yet (candidate next steps)
Per-user goals / TDEE calculator, weight tracking, custom user-created foods,
recipes/meal templates, weekly stats & charts, copy-previous-day, entry editing UI
(mutation exists, no UI).
